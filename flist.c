...
struct file_list *send_file_list(int f, int argc, char *argv[])
{
	...
	int64 start_write;
	// int64 - int64_t - typedef long int/ long long int
	int use_ff_fd = 0;
	...
	if (show_filelist_progress) 
		...
		// diff in no xfer_dirs,it means no transfer dirs, noserver side and inc_recurse == INFO_GTE(FLIST, 1) == 1
	...
	while (1) {
		...
		if (use_ff_fd) {
			...
			// read a line data from filesfrom_fd into buff
				break;
...
{
	/* part 1 send file list */
	struct file_struct *file; // need use
	static const char *lastdir;
	// the memory unit of this lastdir point can't be change
	static int lastdir_len = -1;
	int len, dirlen;
